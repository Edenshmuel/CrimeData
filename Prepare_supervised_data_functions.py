...
def Preprocessing(datasets, train_mappings=None, scaler=None, fit_scaler=False):
    ...
    datasets['is_missing_StatisticArea'] = datasets['StatisticAreaKod'].isnull().astype(int)
    ...
    datasets['StatisticAreaKod'] = datasets.groupby('YeshuvKod')['StatisticAreaKod'].transform(
        lambda x: x.fillna(fill_statistic_area_random(x)))
    datasets = datasets.dropna(subset=['StatisticAreaKod'])
    ...
    datasets['YeshuvCrimeRate'] = datasets.groupby('YeshuvKod')['YeshuvKod'].transform('count')
    ...
    station_crime_rate = datasets.groupby('PoliceStationKod')['YeshuvCrimeRate'].transform('mean')
    datasets['StationCrimeRateAvg'] = station_crime_rate

    # Calculating historical crime rate per Yeshuv
    datasets['YeshuvHistoricalCrimeRate'] = datasets.groupby('YeshuvKod')['YeshuvCrimeRate'].transform('mean')

    # Calculating the number of nearby police stations within the same district
    datasets['StationsNearbyCount'] = datasets.groupby('PoliceDistrictKod')['PoliceStationKod'].transform('nunique')
    ...
    columns_to_remove_after_fill = ['StatisticArea', 'Yeshuv']
    datasets = datasets.drop(columns=columns_to_remove_after_fill, errors='ignore')

    # Remove rows with missing values in police hierarchy columns
    ...
    columns_to_remove_redundant = [
        'PoliceMerhav', 'PoliceDistrict', 'PoliceStation']
